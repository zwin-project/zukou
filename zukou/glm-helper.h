...
static inline void vec3_to_wl_array(glm::vec3 v, struct wl_array *array) {
  size_t size = sizeof(float) * 3;
  float *data = (float *)wl_array_add(array, size);
  memcpy(data, &v, size);
}

static inline void vec4_to_wl_array(glm::vec4 v, struct wl_array *array) {
  size_t size = sizeof(float) * 4;
  float *data = (float *)wl_array_add(array, size);
  memcpy(data, &v, size);
}

static inline void mat4_to_wl_array(glm::mat4 m, struct wl_array *array) {
...
static inline void quat_to_wl_array(
    glm::quat quaternion, struct wl_array *array) {
